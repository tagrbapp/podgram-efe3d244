-- تعديل دالة place_bid لإضافة إشعارات عند تمديد المزاد
...
BEGIN
  ...
  -- التحقق من الوقت المتبقي وتمديد المزاد إذا لزم الأمر
  _time_remaining := _auction.end_time - now();
  
  IF _time_remaining < INTERVAL '5 minutes' THEN
    -- تمديد المزاد بـ 15 دقيقة إضافية
    UPDATE public.auctions
    SET end_time = end_time + INTERVAL '15 minutes'
    WHERE id = _auction_id;
    
    _extended := true;
    
    -- تحديث متغير _auction ليعكس الوقت الجديد
    SELECT * INTO _auction
    FROM public.auctions
    WHERE id = _auction_id;
  END IF;
  ...
  -- إرسال إشعار التمديد لجميع المزايدين والبائع إذا تم تمديد المزاد
  IF _extended THEN
    -- إشعار البائع بالتمديد
    IF _auction_owner_id IS NOT NULL THEN
      INSERT INTO public.notifications (user_id, title, message, type, listing_id)
      VALUES (
        _auction_owner_id,
        '⏰ تمديد المزاد',
        'تم تمديد مزادك "' || COALESCE(_auction_title, 'المزاد') || '" لمدة 15 دقيقة إضافية بسبب مزايدة في آخر 5 دقائق',
        'auction_extended',
        _auction.listing_id
      );
    END IF;
    
    -- إشعار جميع المزايدين بالتمديد
    FOR _bidder IN 
      SELECT DISTINCT user_id 
      FROM public.bids 
      WHERE auction_id = _auction_id AND user_id != _user_id
    LOOP
      INSERT INTO public.notifications (user_id, title, message, type, listing_id)
      VALUES (
        _bidder.user_id,
        '⏰ تمديد المزاد',
        'تم تمديد المزاد على "' || COALESCE(_auction_title, 'المزاد') || '" لمدة 15 دقيقة إضافية! لا تفوت الفرصة',
        'auction_extended',
        _auction.listing_id
      );
    END LOOP;
  ...
  RETURN json_build_object(
    ...
    'current_bid', _bid_amount,
    'extended', _extended,
    'new_end_time', _auction.end_time
  );
END;
$function$;